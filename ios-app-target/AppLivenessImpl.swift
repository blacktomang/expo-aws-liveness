// This file lives in the host app target's source phase, NOT in the
// ExpoAwsLiveness pod. The config plugin copies it from
// modules/expo-aws-liveness/ios-app-target/ to ios/EBuddy/expo-aws-liveness/
// on every prebuild. App targets see SwiftPM packages natively, so the
// `import Amplify` / `import FaceLiveness` lines below resolve correctly
// (whereas pod targets can't satisfy those modules' transitive C-module
// dependencies).
//
// AppDelegate is patched by the same config plugin to register an instance
// of AppLivenessImpl with ExpoAwsLivenessRegistry at app launch.

import Amplify
import AWSCognitoAuthPlugin
import ExpoAwsLiveness  // pod's static framework — gives us the protocol + registry
import FaceLiveness
import Foundation
import SwiftUI
import UIKit

public final class AppLivenessImpl: NSObject, ExpoAwsLivenessImpl {
    ...
    DispatchQueue.main.async {
      guard let presenter = Self.topViewController() else {
        completion(.failure(NSError(
          domain: "NO_PRESENTER",
          code: -1,
          userInfo: [NSLocalizedDescriptionKey: "No UIViewController available to present from."]
        )))
        return
      }
...
/// the pod expects. Error domains use Amplify exception class names so the
